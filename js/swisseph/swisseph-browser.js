// swisseph-browser.js — Swiss Ephemeris for browsers (WASM)
// Patched: removed ES module export to allow classic script loading.
// exports shim for CommonJS-style internal code:
var exports = exports || {};
...
var _SwissEphemeris = class _SwissEphemeris {
  ...
  async init(wasmPath) {
    ...
    // Cargar swisseph.js como script clásico (no es ES module)
    if (!window.SwissEphModule) {
      await new Promise((resolve, reject) => {
        const script = document.createElement('script');
        script.src = './js/swisseph/swisseph.js';
        script.onload = resolve;
        script.onerror = () => reject(new Error('Failed to load swisseph.js'));
        document.head.appendChild(script);
      });
    }
    let SwissEphModuleFactory = window.SwissEphModule;
    ...
    if (!resolvedWasmPath) {
      try {
        resolvedWasmPath = new URL("./swisseph.wasm", document.currentScript ? document.currentScript.src : (window.__swissephBasePath || './js/swisseph/swisseph-browser.js')).href;
      } catch (e) {
  ...
  async loadStandardEphemeris() {
    // Cargar archivos de efemérides desde el servidor local (no CDN, para evitar CORS)
    const base = new URL('./js/swisseph/ephe/', window.location.href).href;
    await this.loadEphemerisFiles([
      { name: "sepl_18.se1", url: `${base}sepl_18.se1` },
      { name: "semo_18.se1", url: `${base}semo_18.se1` },
      { name: "seas_18.se1", url: `${base}seas_18.se1` }
    ]);
  }
  /**
  ...
  julianDayToDate(jd, calendarType = 1) {
    ...
    m._free(dayPtr);
    m._free(hourPtr);
    return { year, month, day, hour, calendarType, toString() { return `${year}-${month}-${day} ${hour}h`; }, toISOString() { return new Date(Date.UTC(year, month-1, day, Math.floor(hour), Math.floor((hour%1)*60))).toISOString(); } };
  }
  /**
   ...
   * );
   */
  calculatePosition(julianDay, body, flags = 4 | 256) {
    this._checkReady();
    // normalizeFlags fallback: si flags es number, usarlo directo
    const normalizedFlags = typeof flags === 'number' ? flags : (typeof flags === 'object' && flags !== null && 'valueOf' in flags) ? flags.valueOf() : 4 | 256;
    const m = this.module;
  ...
  findNextLunarEclipse(startJulianDay, flags = 4, eclipseType = 0, backward = false) {
    this._checkReady();
    const normalizedFlags = typeof flags === 'number' ? flags : 4;
    const normalizedEclipseType = typeof eclipseType === 'number' ? eclipseType : 0;
    const m = this.module;
    ...
    m._free(tretPtr);
    m._free(serrPtr);
    return { type: retflag, maximum: tret[0], partialBegin: tret[1], partialEnd: tret[2], totalBegin: tret[3], totalEnd: tret[4], penumbralBegin: tret[5], penumbralEnd: tret[6], isTotal() { return (retflag & 4) !== 0; }, isPartial() { return (retflag & 16) !== 0; }, isPenumbralOnly() { return (retflag & 64) !== 0; }, getTotalityDuration() { return tret[4] && tret[3] ? (tret[4]-tret[3])*24 : 0; }, getPartialDuration() { return tret[2] && tret[1] ? (tret[2]-tret[1])*24 : 0; }, getTotalDuration() { return tret[6] && tret[5] ? (tret[6]-tret[5])*24 : 0; } };
  }
  /**
  ...
  findNextSolarEclipse(startJulianDay, flags = 4, eclipseType = 0, backward = false) {
    ...
    m._free(tretPtr);
    m._free(serrPtr);
    return { type: retflag, maximum: tret[0], partialBegin: tret[1], partialEnd: tret[2], centralBegin: tret[3], centralEnd: tret[4], centerLineBegin: tret[5], centerLineEnd: tret[6], isTotal() { return (retflag & 4) !== 0; }, isAnnular() { return (retflag & 8) !== 0; }, isHybrid() { return (retflag & 32) !== 0; }, isPartial() { return (retflag & 16) !== 0; }, isCentral() { return (retflag & 1) !== 0; }, isNonCentral() { return (retflag & 2) !== 0; } };
  }
  /**
